...
def _verify_act_owner(sb, week_activity_id: str, user_id: str):
    act = sb.table("week_activities").select("id, weeks!inner(user_id)").eq("id", week_activity_id).execute()
...
async def get_todos(week_activity_id: str, user: dict = Depends(get_current_user)):
    ...
    resp = sb.table("activity_todos").select("*").eq("week_activity_id", week_activity_id).order("created_at").execute()
    return resp.data or []
...
async def update_todo(todo_id: str, body: TodoUpdate, user: dict = Depends(get_current_user)):
    sb = get_supabase()
    uid = user["user_id"]
    existing = sb_single(sb.table("activity_todos").select("id, user_id").eq("id", todo_id))
...
async def get_inbox(user: dict = Depends(get_current_user)):
    sb = get_supabase()
    uid = user["user_id"]
    resp = sb.table("todo_inbox").select("*").eq("user_id", uid).eq("reviewed", False).order("created_at", desc=True).execute()
    return resp.data or []
...
async def review_inbox_item(inbox_id: str, body: dict, user: dict = Depends(get_current_user)):
    sb = get_supabase()
    uid = user["user_id"]
    existing = sb_single(sb.table("todo_inbox").select("id, user_id").eq("id", inbox_id))
    if not existing or existing["user_id"] != uid:
