...
async def update_resource(resource_id: str, body: ResourceUpdate, user: dict = Depends(get_current_user)):
    sb = get_supabase()
    uid = user["user_id"]
    existing = sb_single(sb.table("resources").select("id").eq("id", resource_id).eq("user_id", uid))
    if not existing:
...
async def link_resource(act_id: str, body: dict, user: dict = Depends(get_current_user)):
    ...
    res_existing = sb_single(sb.table("resources").select("id").eq("id", resource_id).eq("user_id", uid))
    if not res_existing:
        raise HTTPException(status_code=404, detail="Resource not found")
    plan_act = sb_single(sb.table("plan_activities").select("id, plans!inner(user_id)").eq("id", act_id))
    week_act = sb_single(sb.table("week_activities").select("id, weeks!inner(user_id)").eq("id", act_id))
    insert_data: dict = {"resource_id": resource_id}
    if plan_act and plan_act.get("plans", {}).get("user_id") == uid:
        insert_data["plan_activity_id"] = act_id
    elif week_act and week_act.get("weeks", {}).get("user_id") == uid:
